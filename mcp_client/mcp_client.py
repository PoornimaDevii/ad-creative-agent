...
async def _call_tool(tool_name: str, arguments: dict, context_id: str = None) -> Any:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        logger.info(f"[CALL] tool={tool_name} context_id={context_id} attempt={attempt}/{MAX_RETRIES} args={list(arguments.keys())}")
        try:
            async with sse_client(MCP_SERVER_URL) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    if not result.content:
                        logger.warning(f"[EMPTY] tool={tool_name} context_id={context_id}")
                        return {}
                    parsed = json.loads(result.content[0].text)
                    if isinstance(parsed, dict) and parsed.get("status") == "error":
                        logger.warning(f"[ERROR_STATUS] tool={tool_name} context_id={context_id} errors={parsed.get('errors', [])}")
                        return parsed
                    logger.info(f"[OK] tool={tool_name} context_id={context_id} keys={list(parsed.keys()) if isinstance(parsed, dict) else 'list'}")
                    return parsed
        except Exception as e:
            last_error = e
            logger.warning(f"[RETRY] tool={tool_name} context_id={context_id} attempt={attempt} error={type(e).__name__}: {e}")
            if attempt < MAX_RETRIES:
                await asyncio.sleep(RETRY_DELAY * attempt)

    logger.error(f"[FAILED] tool={tool_name} context_id={context_id} after {MAX_RETRIES} attempts. Last error: {last_error}")
...
async def _read_resource(uri: str, context_id: str = None) -> Any:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        logger.info(f"[READ_RESOURCE] uri={uri} context_id={context_id} attempt={attempt}/{MAX_RETRIES}")
        try:
            async with sse_client(MCP_SERVER_URL) as (read, write):
                async with ClientSession(read, write) as session:
                    await session.initialize()
                    ...
                    if not result.contents:
                        logger.warning(f"[EMPTY_RESOURCE] uri={uri} context_id={context_id}")
                        return {}
                    parsed = json.loads(result.contents[0].text)
                    logger.info(f"[OK_RESOURCE] uri={uri} context_id={context_id}")
                    return parsed
        except Exception as e:
            last_error = e
            logger.warning(f"[RETRY_RESOURCE] uri={uri} context_id={context_id} attempt={attempt} error={type(e).__name__}: {e}")
            if attempt < MAX_RETRIES:
                await asyncio.sleep(RETRY_DELAY * attempt)

    logger.error(f"[FAILED_RESOURCE] uri={uri} context_id={context_id} after {MAX_RETRIES} attempts. Last error: {last_error}")
    return {}
